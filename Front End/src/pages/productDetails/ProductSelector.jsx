            ...
            {(product.productFlavors?.length > 0 ? product.productFlavors.filter(pf => pf.Flavor) : flavors).map(item => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 px-6 py-4 rounded-3xl border-2 transition-all duration-300 ${selectedFlavorId === fId
                ...
                >
                  <div className="w-6 h-6 rounded-full overflow-hidden border border-slate-100 flex-shrink-0">
                    {f.image ? (
                      <img src={f.image.startsWith('http') ? f.image : `${API_BASE_URL}${f.image}`} className="w-full h-full object-cover" alt="" />
                    ) : (
      ...
      <div>
        ...
        <div className="space-y-3">
          {sizeOptions.map((size) => (
            <div
              key={size.id}
              className={`p-5 rounded-3xl border-2 cursor-pointer transition-all duration-300 ${selectedSize === size.id ? 'border-sky-500 bg-sky-50/50 shadow-xl' : 'border-slate-100 hover:border-slate-200'
                }`}
